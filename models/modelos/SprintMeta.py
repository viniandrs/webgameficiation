...
class SprintMeta(ItemDeTrabalho):
    ...
    def adicionar_tarefa(self, tarefa: Tarefa):
        if not isinstance(tarefa, Tarefa):
            raise TypeError(
                "O objeto a ser adicionado a lista de tarefas do Sprint Meta deve ser um objeto Tarefa"
            )  # possivelmente substituir por um tratamento de erro amis adequado posteriormente
        else:
    ...
    def remover_tarefa(self, tarefa: Tarefa):
        if tarefa in self.__tarefas:
            ...
            raise ValueError(
                "A tarefa não está associada a este SprintMeta"
            )  # possivelmente substituir por um tratamento de erro mais adequado posteriormente
    ...
    def contribuir_para_projeto(self, projeto: Projeto):
        if projeto.get_id() == self.get_projeto_id():
            if all(
                tarefa.get_status() == StatusItem.CONCLUIDA for tarefa in self.__tarefas
            ):
