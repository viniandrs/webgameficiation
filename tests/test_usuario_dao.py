...
from src.utilidades_banco_dados import obter_conexao_teste
from src.configuracao_banco_dados import criar_tabelas_em_conexao, apagar_tabelas_em_conexao
...
class TestUsuarioDao(unittest.TestCase):
  ...
  def setUp(self):
      ...
      self.patcher_obter_conexao = patch('src.acesso_dados.BaseDao.obter_conexao', return_value= self.conexao_teste)
  ...
  def test_inserir_usuario(self):
    ...
    self.assertIsNotNone(usuario_inserido.get_id()) #Verifica se um id foi atribuído
  ...
  def test_remover_usuario(self):
    ...
    usuario_a_remover = self.dao.inserir(Usuario("Removido", "removido@gmail.com", "senha"))
